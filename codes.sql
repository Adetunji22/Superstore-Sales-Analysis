...
2b.  ----calculating the profit percentage for each category
  with temp as(
select category, sum(sales)::money as revenue, sum(profit)::money as profits
from us_superstore group by category )
 select *, round((profits::numeric/revenue::numeric)*100,2) as profit_percentage from temp
...
8.  ----total revenue generated per year for each subcategory
 ...
 10.   -- year over year revenue pct change
    with yoy as(
select extract(year from order_date)as years, sum(sales)::money as revenue,
lag(sum(sales))over(order by extract(year from order_date) )::money as previous_revenue
from US_superstore group by years order by years)
select years, revenue, previous_revenue,
(((revenue-previous_revenue)/previous_revenue)*100)as year_over_year_pct_change
from yoy
